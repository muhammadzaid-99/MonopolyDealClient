  ...
  return (
    <div className="flex flex-col items-center justify-center gap-4 h-full bg-white ">
      <div className="flex flex-col w-full rounded-md border shadow-inner h-full">
        {/* Discard Pile */}
        <ScrollArea className="h-32 bg-gray-50 flex flex-col gap-1 border text-center">
          {gameInfo?.messages?.map((msg, i) => (
            <div key={i} className="text-xs p-2 border-b select-none tracking-wide font-serif">
              {msg}
            </div>
          ))}
        </ScrollArea>
        <div className="p-4 flex flex-1 items-center justify-center text-xs">
          {gameInfo?.top_card ? (
        ...
        <div className="flex gap-1 items-center bg-gray-100 w-full justify-between">
          {/* Draw Pile */}
          <Button className={`bg-slate-700 h-8 m-4 rounded-md border self-end text-xs text-center shadow-inner ${playerID === gameInfo.current_turn_player_id && (!gameInfo.turn_cards_drawn || action == Action.DrawCards) && "animate-pulse"}`} onClick={HandleDrawCards}>
            Draw Cards
          </Button>
          <div className="text-xs font-semibold text-center [text-wrap:balance]">
            {ActionSentence(action)}
          </div>
          <Button variant="secondary" className={`h-8 m-4 rounded-md border self-end text-xs text-center shadow-inner ${"animate-pulse duration-1000"}`}
